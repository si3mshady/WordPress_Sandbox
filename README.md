...
```bash
wordpress_blog$ sam build --use-container
```
...
```bash
wordpress_blog$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
wordpress_blog$ sam logs -n HelloWorldFunction --stack-name wordpress_blog --tail
```
...
```bash
wordpress_blog$ pip install -r tests/requirements.txt --user
# unit test
wordpress_blog$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
wordpress_blog$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name wordpress_blog
```
